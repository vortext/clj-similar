...
class KDNode<T> implements Serializable {
  ...
  protected static <T> int edit(final HPoint key, final Editor<T> editor, final KDNode<T> t, final int lev, final int K) throws KeyDuplicateException {
    KDNode<T> next_node = null;
    final int next_lev = (lev + 1) % K;
    synchronized (t) {
      if (key.equals(t.k)) {
        final boolean was_deleted = t.deleted;
  ...
  protected static <T> KDNode<T> create(final HPoint key, final Editor<T> editor) throws KeyDuplicateException {
    final KDNode<T> t = new KDNode<T>(key, editor.edit(null));
    if (t.v == null) {
      t.deleted = true;
    }
    return t;
  ...
  protected static <T> boolean del(final KDNode<T> t) {
    synchronized (t) {
      if (!t.deleted) {
        t.deleted = true;
        return true;
  ...
  protected static <T> KDNode<T> srch(final HPoint key, KDNode<T> t, final int K) {
  ...
  protected static <T> void rsearch(final HPoint lowk, final HPoint uppk, final KDNode<T> t, final int lev, final int K, final List<KDNode<T>> v) {
  ...
  protected static <T> void nnbr(final KDNode<T> kd, final HPoint target, final HRect hr, double max_dist_sqd, final int lev, final int K,
      final NearestNeighborList<KDNode<T>> nnl, final Checker<T> checker, final long timeout) {
  ...
  private KDNode(final HPoint key, final T val) {
  ...
  private static void hrcopy(final HRect hr_src, final HRect hr_dst) {
    hpcopy(hr_src.min, hr_dst.min);
    hpcopy(hr_src.max, hr_dst.max);
  }

  private static void hpcopy(final HPoint hp_src, final HPoint hp_dst) {
