...
class NearestNeighborList<T> {
  ...
  static class NeighborEntry<T> implements Comparable<NeighborEntry<T>> {
    ...
    public int compareTo(final NeighborEntry<T> t) {
  ...
  public double getMaxPriority() {
    final NeighborEntry p = m_Queue.peek();
    return (p == null) ? Double.POSITIVE_INFINITY : p.value;
  }

  public boolean insert(final T object, final double priority) {
  ...
  public T getHighest() {
    final NeighborEntry<T> p = m_Queue.peek();
  ...
  public T removeHighest() {
    // remove object with highest priority
    final NeighborEntry<T> p = m_Queue.poll();
    return (p == null) ? null : p.data;
  }
}
